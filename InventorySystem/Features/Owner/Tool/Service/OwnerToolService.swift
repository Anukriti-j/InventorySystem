...
final class OwnerToolService {
    ...
    func deleteTool(toolID: Int) async throws -> DeleteToolResponse {
        let path = pathBuilder.buildPath("/tool/delete/\(toolID)")
    ...
    func getCategories() async throws -> GetCategories {
        let endpoint = APIEndpoint(
            path: "\(APIConstants.baseURL)/tools/category/all",
            method: .get,
            requiresAuth: true
        )
        return try await APIClient.shared.request(endpoint: endpoint, responseType: GetCategories.self)
    }
}
